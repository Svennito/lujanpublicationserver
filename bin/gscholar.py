...
#
# To do:
# - save entries, also with query string and instrument as fields
# - catch 503 service not available error
# 
...
def query(searchstr, year_from=False, year_to=False, exact_phrase=False, allresults=True):
    ...
    searchstr = searchstr+"&as_oq=lanl+%22los+alamos%22+lansce&as_eq=&as_occt=any&as_sauthors=&as_publication="
...
if __name__ == "__main__":
	...
	# Do we need to query from a year only?
	  ...
	  logging.debug("Querying publications to year: %s." % options.query_year_to)
        biblist = query(args, options.query_year_from, options.query_year_to, options.exact_phrase, options.all)
    if len(biblist) < 1:
